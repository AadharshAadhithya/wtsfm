...
def _resolve_data_file(data_path: Path, version: str) -> Path:
    """Return path to the RML archive file; accept either a file path or a dataset root directory."""
    data_path = Path(data_path)
    if data_path.is_file():
        return data_path
    if not data_path.is_dir():
        raise FileNotFoundError(f"RML data path does not exist: {data_path}")
    if version == "2022":
        candidates = [
            data_path / "RML22.01A",
            data_path / "RML22.01A.npy",
            data_path / "RML22.01a.npy",
        ]
    else:
        candidates = [
            data_path / "RML2016.10a_dict.pkl",
            data_path / "RML2016.10b_dict.pkl",
            data_path / "RML2016.10a_dict.pickle",
        ]
    for c in candidates:
        if c.is_file():
            return c
    tried = ", ".join(str(c.name) for c in candidates)
    raise FileNotFoundError(
        f"No RML {version} archive under {data_path}. "
        f"Tried: {tried}. Download the dataset into this folder or pass the full path to the archive file."
    )
...
def preprocess_rml(
    ...
    data_file = _resolve_data_file(data_file, version)
    output = Path(output)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        dest="data_file",
        required=True,
        help="Path to the archive file, or a folder containing it (e.g. …/RML22.01A or …/RML2022/).",
    )
